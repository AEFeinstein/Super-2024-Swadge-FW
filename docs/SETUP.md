...
6. Clone the ESP-IDF v5.3.1 and install the tools. Note that it will clone into `$HOME/esp/esp-idf`.
Note: Some installs of Python will have py.exe instead of python.exe - If this is the case, you can edit install.ps1 to replace all instances of python.exe to py.exe OR rename your locally installed py.exe file to python.exe
    ...
    & git clone -b v5.3.1 --recurse-submodules https://github.com/espressif/esp-idf.git $HOME/esp/esp-idf
...
3. Install `clang-17` and `clang-format-17` separately. Note that the version used in this project is currently 17 and the version in many package managers is less than that.
    ...
    ```
4. Clone the ESP-IDF v5.3.1 and install the tools. Note that it will clone into `~/esp/esp-idf`.
    ```bash
...
4. Run the following command to install all necessary dependencies:
    ...
    ```
5. Clone the ESP-IDF v5.3.1 and install the tools. Note that it will clone into `~/esp/esp-idf`.
    ```bash
    git clone -b v5.3.1 --recurse-submodules https://github.com/espressif/esp-idf.git ~/esp/esp-idf
    ~/esp/esp-idf/install.sh
    ```
