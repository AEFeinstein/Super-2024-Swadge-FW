...
8. Clone the ESP-IDF v5.0.2 and install the tools. Note that it will clone into `$HOME/esp/esp-idf`.
    ```powershell
    & Set-ExecutionPolicy -Scope CurrentUser Unrestricted
    & git clone -b v5.0.2 --recurse-submodules https://github.com/espressif/esp-idf.git $HOME/esp/esp-idf
...
1. Run the following commands, depending on your package manager, to install all necessary packages:
    ...
    * `dnf`:
        ...
        ```
2. Clone the ESP-IDF v5.0.2 and install the tools. Note that it will clone into `~/esp/esp-idf`.
    ```bash
    git clone -b v5.0.2 --recurse-submodules https://github.com/espressif/esp-idf.git ~/esp/esp-idf
