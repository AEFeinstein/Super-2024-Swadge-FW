...
swadgeMode_t* const allSwadgeModes[] = {
    &accelTestMode,      &artilleryMode,    &atriumMode,     &bouncyMode,         &canvasTestMode,
    &cipherMode,         &colorchordMode,   &cosCrunchMode,  &danceMode,          &danceNetworkMode,
    &factoryTestMode,    &findingFacesMode, &gamepadMode,    &introMode,          &jukeboxMode,
    &keebTestMode,       &lpMode,           &mainMenuMode,   &modeCh32v003test,   &modeCredits,
    &modeDiceRoller,     &modePicross,      &modePlatformer, &nameTestMode,       &roboRunnerMode,
    &sequencerMode,      &sonaTestMode,     &swadgeItMode,   &swadgePassTestMode, &swadgedokuMode,
    &swadgetamatoneMode, &swsnCreatorMode,  &synthMode,      &tCaseMode,          &touchTestMode,
...
void modeListSetMenu(menu_t* menu)
{
    ...
    addSingleItemToMenu(menu, lpMode.modeName);
