...
static void flightUpdateLEDs(flight_t * tflight)
{
    ...
    case FLIGHT_LED_BEAN:
        ...
        leds[2] = leds[5] = LedEHSVtoHEXhelper(ledAnimationTime*16, 128, 150 - 40*abs(ledAnimationTime-10), 1 );
        leds[3] = leds[4] = LedEHSVtoHEXhelper(ledAnimationTime*16, 128, 150 - 40*abs(ledAnimationTime-14), 1 );
        if( ledAnimationTime == 30 ) flightLEDAnimate( FLIGHT_LED_NONE );
        break;
    }
...
static void flightStartGame( flightModeScreen mode )
{
    ...
    flight->lastFlightUpdate = flight->timeOfStart = (uint32_t)esp_timer_get_time();//-1000000*190; // (Do this to force extra coursetime)
    ...
    else if( m->indices_per_face == 3 )
    {
        for( i = 0; i < nri; i+=3 )
        {
            ...
            if( cv1[2] != 2 && cv2[2] != 2 && cv3[2] != 2 )
            {
                ...
                int Vy = cv2[1] - cv1[1];
                if( Ux*Vy-Uy*Vx >= 0 )
                    drawTriangleOutlined( cv1[0], cv1[1], cv2[0], cv2[1], cv3[0], cv3[1], flight->bgcolor, flight->renderlinecolor );
            }
        }
    }
}
...
// #endif
    ...
    if( flight->mode == FLIGHT_GAME || tflight->mode == FLIGHT_FREEFLIGHT )
    {
        ...
        if( flight->mode == FLIGHT_FREEFLIGHT )
        {
            ...
            width = textWidth(&flight->radiostars, framesStr);
            drawText(&flight->radiostars, PROMPT_COLOR, framesStr, (TFT_WIDTH-width)/2, TFT_HEIGHT - flight->radiostars.height );
        }

        snprintf(framesStr, sizeof(framesStr), "%d", tflight->speed);
        width = textWidth(&flight->radiostars, framesStr);
        fillDisplayArea(TFT_WIDTH - width-50, TFT_HEIGHT - flight->radiostars.height - 1, TFT_WIDTH, TFT_HEIGHT, CNDRAW_BLACK);
        drawText(&flight->radiostars, PROMPT_COLOR, framesStr, TFT_WIDTH - width + 1-50, TFT_HEIGHT - flight->radiostars.height );


        if(flight->paused)
        {
            width = textWidth(&flight->ibm, fl_paused);
            fillDisplayArea((TFT_WIDTH - width) / 2 - 2, TFT_HEIGHT / 4 - 2, (TFT_WIDTH + width) / 2 + 2, TFT_HEIGHT / 4 + flight->ibm.height + 2, CNDRAW_BLACK);
        ...
        if(flight->oob)
        {
            ...
            drawText(&flight->ibm, PROMPT_COLOR, fl_turn_around, (TFT_WIDTH - width) / 2, (TFT_HEIGHT - 6 * flight->ibm.height) / 2);
        }
    ...
    else
    {
        char framesStr[32] = {0};
        //ets_snprintf(framesStr, sizeof(framesStr), "%02x %dus", tflight->buttonState, (stop-start)/160);
        drawText(&flight->radiostars, PROMPT_COLOR, fl_you_win, (TFT_WIDTH - textWidth(&flight->radiostars, fl_you_win)) / 2, 50);
        snprintf(framesStr, sizeof(framesStr), "TIME: %d.%02d", tflight->wintime/100,tflight->wintime%100 );
        drawText(&flight->radiostars, PROMPT_COLOR, framesStr, (TFT_WIDTH - textWidth(&flight->radiostars, framesStr)) / 2/*((tflight->wintime>10000)?14:20)+75*/, 18+50);
        snprintf(framesStr, sizeof(framesStr), "BEANS: %2d",tflight->beans );
        drawText(&flight->radiostars, PROMPT_COLOR, framesStr, (TFT_WIDTH - textWidth(&flight->radiostars, framesStr)) / 2, 36+50);
    }
    ...
    if( tflight->beans >= MAX_BEANS )
    {
        ...
        drawText(&flight->ibm, crazy?( tflight->tframes * 9 % MAX_COLOR ):PROMPT_COLOR, fl_100_percent, (TFT_WIDTH - textWidth(&flight->ibm, fl_100_percent)) / 2, (TFT_HEIGHT - 4 * flight->ibm.height) / 2 + 2);
    }
    ...
    switch (flight->mode)
    {
        ...
        case FLIGHT_MENU:
        {
            if(down)
            {
                ...
                {
                    flight->savedata.flightInvertY ^= 1;
                    flight->menuEntryForInvertY->label = flight->savedata.flightInvertY?fl_flight_invertY1_env:fl_flight_invertY0_env;
                                   ...
                                   int8_t rssi)
{
    ...
    uint32_t pcode = *((const uint32_t*)data);  data+= 4;
