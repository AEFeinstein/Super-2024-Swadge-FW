...
static const int16_t bombExplosionTileCheckOffsets[74] = {
    //   X,  Y
    -1, -3, 0,  -3, 1,  -3, -2, -2, -1, -2, 0,  -2, 1, -2, 2, -2, -3, -1, -2, -1, -1, -1, 0, -1, 1,
...
void updatePlayer(entity_t* self)
{
    if (self->gameData->isTouched)
    {
        ...
        int32_t touchIntoLevel
            = (self->gameData->touchX << 2) + 192; // play with this value until center touch moves paddle to center

        //                                    the leftmost coordinate that the originX point of the paddle sprite can
        //                                    occupy |   the rightmost coordinate that the originX point of the paddle
        //                                    sprite can occupy
...
void updatePlayerVertical(entity_t* self)
{
    if (self->gameData->isTouched)
    {
        ...
        int32_t touchIntoLevel = ((984 - self->gameData->touchY) << 2)
                                 + 128; // play with this value until center touch moves paddle to center
...
void updateBall(entity_t* self)
{
        ...
        switch (self->attachedToEntity->type)
        {
            case (ENTITY_PLAYER_PADDLE_BOTTOM):
                ...
                self->y = self->attachedToEntity->y
                          - ((self->entityManager->sprites[self->spriteIndex].originY
                              + self->entityManager->sprites[self->attachedToEntity->spriteIndex].originY)
                             << SUBPIXEL_RESOLUTION);
            ...
            case (ENTITY_PLAYER_PADDLE_LEFT):
                self->x = self->attachedToEntity->x
                          + ((self->entityManager->sprites[self->spriteIndex].originX
                              + self->entityManager->sprites[self->attachedToEntity->spriteIndex].originX)
                             << SUBPIXEL_RESOLUTION);
                self->y = self->attachedToEntity->y;
    ...
    else
    {
        ...
        if (self->gameData->btnState & PB_DOWN && !(self->gameData->prevBtnState & PB_DOWN))
        {
            if (self->gameData->playerTimeBombsCount < 3)
            {
                // Drop time bomb
                entity_t* createdBomb = createEntity(self->entityManager, ENTITY_PLAYER_TIME_BOMB,
        ...
        if (self->gameData->btnState & PB_RIGHT && !(self->gameData->prevBtnState & PB_RIGHT))
        {
            ...
            {
                // Drop remote bomb
                entity_t* createdBomb = createEntity(self->entityManager, ENTITY_PLAYER_REMOTE_BOMB,
                                                     self->x >> SUBPIXEL_RESOLUTION, self->y >> SUBPIXEL_RESOLUTION);
                if (createdBomb != NULL)
                {
        ...
        {
            createEntity(self->entityManager, ENTITY_BALL_TRAIL, self->x >> SUBPIXEL_RESOLUTION,
...
void updateBallAtStart(entity_t* self)
{
    ...
    for (uint8_t i = 0; i < MAX_ENTITIES; i++)
    {
        ...
        if (checkEntity->active && checkEntity != self
            && (checkEntity->type == ENTITY_PLAYER_PADDLE_BOTTOM || checkEntity->type == ENTITY_PLAYER_PADDLE_TOP
                || checkEntity->type == ENTITY_PLAYER_PADDLE_LEFT || checkEntity->type == ENTITY_PLAYER_PADDLE_RIGHT))
        {
...
void updateChoIntro(entity_t* self)
{
    ...
    if (self->attachedToEntity == NULL)
    {
        ...
        if (self->attachedToEntity != NULL)
        {
            ...
            self->y                    = self->attachedToEntity->y;
            self->x                    = ((self->attachedToEntity->x >> SUBPIXEL_RESOLUTION) > (TFT_WIDTH >> 1))
                                             ? 0
                                             : TFT_WIDTH << SUBPIXEL_RESOLUTION;
...
void moveEntityWithTileCollisions(entity_t* self)
{
    ...
    else
    {
        if (self->yspeed != 0)
        {
            ...
            uint8_t newTy = TO_TILE_COORDS(((self->y + self->yspeed) >> SUBPIXEL_RESOLUTION)
                                           + SIGNOF(self->yspeed) * HALF_TILE_SIZE);
        ...
        if (self->xspeed != 0)
        {
            ...
            uint8_t newTx = TO_TILE_COORDS(((self->x + self->xspeed) >> SUBPIXEL_RESOLUTION)
                                           + SIGNOF(self->xspeed) * HALF_TILE_SIZE);
...
void detectEntityCollisions(entity_t* self)
{
    ...
    for (uint8_t i = 0; i < MAX_ENTITIES; i++)
    {
        ...
        if (checkEntity->active && checkEntity != self)
        {
            ...
            checkEntityBox.x0
                = (checkEntity->x >> SUBPIXEL_RESOLUTION) - checkEntitySprite->originX + checkEntitySpriteBox->x0;
            checkEntityBox.y0
                = (checkEntity->y >> SUBPIXEL_RESOLUTION) - checkEntitySprite->originY + checkEntitySpriteBox->y0;
            checkEntityBox.x1
                = (checkEntity->x >> SUBPIXEL_RESOLUTION) - checkEntitySprite->originX + checkEntitySpriteBox->x1;
            checkEntityBox.y1
                = (checkEntity->y >> SUBPIXEL_RESOLUTION) - checkEntitySprite->originY + checkEntitySpriteBox->y1;
...
void advanceBallSpeed(entity_t* self, uint16_t factor)
{
    ...
    if (self->bouncesToNextSpeedUp < 0)
    {
        ...
        self->bouncesToNextSpeedUp = ballSpeedUps[(self->speedUpLookupIndex * BALL_SPEED_UP_TABLE_ROW_LENGTH)
                                                  + BOUNCE_THRESHOLD_LOOKUP_OFFSET];

        self->baseSpeed
            = ballSpeedUps[(self->speedUpLookupIndex * BALL_SPEED_UP_TABLE_ROW_LENGTH) + NEW_SPEED_LOOKUP_OFFSET];
        ...
        switch (direction)
        {
            ...
            case 1: // RIGHT
                if (self->bouncesOffUnbreakableBlocks > self->breakInfiniteLoopBounceThreshold)
                {
                    ...
                    self->yspeed += (1 << SUBPIXEL_RESOLUTION) * ((self->yspeed == 0) ? SIGNOF(self->xspeed) : SIGNOF(self->yspeed));
                    self->bouncesOffUnbreakableBlocks = 0;
                }
                self->xspeed = -self->xspeed;
                break;
            ...
            case 4: // DOWN
                ...
                {
                    // Subtly change ball bounce angle if the ball has bounced off many unbreakable blocks in a row
                    self->xspeed += (1 << SUBPIXEL_RESOLUTION) * ((self->xspeed == 0) ? SIGNOF(self->yspeed) : SIGNOF(self->xspeed));
                    self->bouncesOffUnbreakableBlocks = 0;
                }
                self->yspeed = -self->yspeed;
                break;
