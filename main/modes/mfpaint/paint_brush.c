...
{
    drawCircleFilledScaled(points[0].x, points[0].y, size, col, canvas->x, canvas->y, canvas->xScale, canvas->yScale);
                              ...
                              paletteColor_t col)
{
    ...
    drawRectFilledScaled(x0, y0, x1 + 1, y1 + 1, col, canvas->x, canvas->y, canvas->xScale, canvas->yScale);
}
