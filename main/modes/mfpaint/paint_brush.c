...
{
    plotRectFilledScaled(points[0].x - (size / 2), points[0].y - (size / 2), points[0].x + ((size + 1) / 2), points[0].y + ((size + 1) / 2), col, canvas->x, canvas->y, canvas->xScale, canvas->yScale);
}

void paintDrawCirclePen(paintCanvas_t* canvas, point_t* points, uint8_t numPoints, uint16_t size, paletteColor_t col)
{
    plotCircleFilledScaled(points[0].x, points[0].y, size, col, canvas->x, canvas->y, canvas->xScale, canvas->yScale);

    // fill out the circle if it's very small
    if (size == 1)
    {
        setPxScaled(points[0].x, points[0].y - 1, col, canvas->x, canvas->y, canvas->xScale, canvas->yScale);
        setPxScaled(points[0].x, points[0].y + 1, col, canvas->x, canvas->y, canvas->xScale, canvas->yScale);
    }
}
...
void paintDrawLine(paintCanvas_t* canvas, point_t* points, uint8_t numPoints, uint16_t size, paletteColor_t col)
{
    for (int16_t x = -size / 2; x < (size + 1) / 2; x++)
    {
        ...
        {
            plotLineScaled(points[0].x + x, points[0].y + y, points[1].x + x, points[1].y + y, col, 0, canvas->x, canvas->y, canvas->xScale, canvas->yScale);
        }
    }
}
...
void paintDrawCurve(paintCanvas_t* canvas, point_t* points, uint8_t numPoints, uint16_t size, paletteColor_t col)
{
    for (int16_t x = -size / 2; x < (size + 1) / 2; x++)
    {
        for (int16_t y = -size / 2; y < (size + 1) / 2; y++)
        {
            plotCubicBezierScaled(points[0].x + x, points[0].y + y, points[1].x + x, points[1].y + y, points[2].x + x, points[2].y + y, points[3].x + x, points[3].y + y, col, canvas->x, canvas->y, canvas->xScale, canvas->yScale);
        }
    }
}
...
void paintDrawFilledRectangle(paintCanvas_t* canvas, point_t* points, uint8_t numPoints, uint16_t size, paletteColor_t col)
{
    ...
    plotRectFilledScaled(x0, y0, x1 + 1, y1 + 1, col, canvas->x, canvas->y, canvas->xScale, canvas->yScale);
}
...
void paintDrawCircle(paintCanvas_t* canvas, point_t* points, uint8_t numPoints, uint16_t size, paletteColor_t col)
{
    ...
    for (int16_t x = -size / 2; x < (size + 1) / 2; x++)
    {
        for (int16_t y = -size / 2; y < (size + 1) / 2; y++)
        {
            plotCircleScaled(points[0].x + x, points[0].y + y, r, col, canvas->x, canvas->y, canvas->xScale, canvas->yScale);
        }
    }
}
...
void paintDrawFilledCircle(paintCanvas_t* canvas, point_t* points, uint8_t numPoints, uint16_t size, paletteColor_t col)
{
    ...
    plotCircleFilledScaled(points[0].x, points[0].y, r, col, canvas->x, canvas->y, canvas->xScale, canvas->yScale);
}
...
void paintDrawEllipse(paintCanvas_t* canvas, point_t* points, uint8_t numPoints, uint16_t size, paletteColor_t col)
{
    ...
    for (int16_t x = 0; x < size; x++)
    {
        for (int16_t y = 0; y < size; y++)
        {
            ...
            plotEllipseRectScaled(x0 + x, y0 + y, x1 - x, y1 - y, col, canvas->x, canvas->y, canvas->xScale, canvas->yScale);
