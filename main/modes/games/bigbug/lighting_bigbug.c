...
// Functions
//==============================================================================
inline uint8_t bb_midgroundLighting(wsg_t* headlampWsg, vec_t* lookup, int32_t* garbotnikRotation)
{
    return 0;
}
...
inline uint8_t bb_foregroundLighting(wsg_t* headlampWsg, vec_t* lookup, int32_t* garbotnikRotation)
{
    ...
    if (*garbotnikRotation < -720)
    {
        ...
        {
            uint32_t rgbCol
                = paletteToRGB(headlampWsg->px[(lookup->y * headlampWsg->w) + lookup->x]);
            r = (rgbCol >> 16) & 0xFF;
        }
        int32_t g = 0;
