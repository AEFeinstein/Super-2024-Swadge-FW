...
vec_t bb_collisionCheck(bb_tilemap_t* tilemap, bb_entity_t* ent, vec_t* previousPos){
    ...
    if (best_i > -1)
    {
            ...
            && ent->pos.y - 192 < tilePos.y + BITSHIFT_HALF_TILE)
        {
            ...
            //if previousPos was provided then go ahead and calculate a hit normal.
            if(previousPos != NULL){
                // Resolve garbotnik's position somewhat based on his position previously.
                vec_t normal = subVec2d(*previousPos, tilePos);
                ...
                if ((normal.x < 0 ? -normal.x : normal.x) > (normal.y < 0 ? -normal.y : normal.y))
                {
                    if (normal.x > 0)
                    {
                        normal.x               = 1;
                        normal.y               = 0;
                        ent->pos.x = tilePos.x + 192 + BITSHIFT_HALF_TILE;
                    }
                    else
                    {
                        normal.x               = -1;
                ...
                else
                {
                    if (normal.y > 0)
                    {
                        normal.x               = 0;
                        normal.y               = 1;
                        ent->pos.y = tilePos.y + 192 + BITSHIFT_HALF_TILE;
                    }
                    else
                    {
                        normal.x               = 0;
                ...
                return normal;//return a hit normal for the entity that cares
            }
            return (vec_t){1,1};//return some BS to indicate a collision
        }
        return NULL;//NULL indicates no collision
    }
