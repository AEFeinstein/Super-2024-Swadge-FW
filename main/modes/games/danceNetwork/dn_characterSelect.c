    ...
    if (evt->down)
    {
        switch (evt->button)
        {
            ...
            case PB_LEFT:
            {
                ...
                gameData->xSelectScrollOffset -= (gameData->sprites.groundTile.w);
                break;
            }
            case PB_RIGHT:
            {
...
void dn_DrawCharacterSelect(dn_gameData_t* gameData, int64_t elapsedUs)
{
    ...
    while (xOff > 0)
    {
        xOff -= gameData->sprites.groundTile.w;
        pIdx--;
    }
    ...
    while (xOff < TFT_WIDTH)
    {
        ...
        drawWsgSimple(&gameData->characterAssets[pIdx].kingDown, xOff, yOff - 50);
        drawWsgSimple(&gameData->characterAssets[pIdx].kingUp,   xOff, yOff + gameData->sprites.groundTile.h * 4 - 40);
    ...
    {
        int16_t arrowY = TFT_HEIGHT > 1 - (gameData->font_rodin.height / 2);

        // Draw arrows to indicate this can be scrolled
        drawText(&gameData->font_rodin, c000, "<", 0, arrowY);
        drawText(&gameData->font_rodin, c000, ">", TFT_WIDTH - textWidth(&gameData->font_rodin, ">"), arrowY);
    }
}