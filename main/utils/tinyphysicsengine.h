...
#define TPE_JOINT_SIZE(joint) ((joint).sizeDivided * TPE_JOINT_SIZE_MULTIPLIER)
    ...
    #define TPE_APPROXIMATE_LENGTH                \
        0 /**< whether or not use length/distance \
    ...
    /** When a body is activated by a collision, its deactivation counter will be
      ...
      rest. */
    #define TPE_LIGHT_DEACTIVATION (TPE_DEACTIVATE_AFTER - TPE_DEACTIVATE_AFTER / 10)
#endif
    ...
    /** Connection tension threshold after which twice as much acceleration will
      ...
      environment.*/
    #define TPE_TENSION_GREATER_ACCELERATION_THRESHOLD (TPE_TENSION_ACCELERATION_THRESHOLD * 3)
#endif
...
#define TPE_BODY_FLAG_DEACTIVATED               \
    1 /**< Not being updated due to low energy, \
           "sleeping", will be woken by         \
           collisions etc. */
#define TPE_BODY_FLAG_NONROTATING             \
    2 /**< When set, the body won't rotate,   \
           will only move linearly. Here the  \
           velocity of the body's first joint \
           is the velocity of the whole       \
           body. */
#define TPE_BODY_FLAG_DISABLED          \
    4 /**< Disabled, not taking part in \
           simulation. */
#define TPE_BODY_FLAG_SOFT                   \
    8 /**< Soft connections, effort won't be \
           made to keep the body's shape. */
#define TPE_BODY_FLAG_SIMPLE_CONN              \
    16 /**< Simple connections, don't zero out \
            antagonist forces or apply         \
            connection friction, can increase  \
            performance. */
#define TPE_BODY_FLAG_ALWAYS_ACTIVE          \
    32 /**< Will never deactivate due to low \
            energy. */
...
typedef uint8_t (*TPE_CollisionCallback)(uint16_t, uint16_t, uint16_t, uint16_t, TPE_Vec3);
...
uint8_t TPE_testClosestPointFunction(TPE_ClosestPointFunction f, TPE_Vec3 cornerFrom, TPE_Vec3 cornerTo,
                                     uint8_t gridResolution, TPE_UnitReduced allowedError, TPE_Vec3* errorPoint);

void TPE_bodyInit(TPE_Body* body, TPE_Joint* joints, uint8_t jointCount, TPE_Connection* connections,
                  uint8_t connectionCount, TPE_Unit mass);
...
TPE_Vec3 TPE_bodyGetRotation(const TPE_Body* body, uint16_t joint1, uint16_t joint2, uint16_t joint3);
...
static inline TPE_Unit TPE_connectionTension(TPE_Unit length, TPE_Unit desiredLength);
...
uint8_t TPE_jointsResolveCollision(TPE_Joint* j1, TPE_Joint* j2, TPE_Unit mass1, TPE_Unit mass2, TPE_Unit elasticity,
...
uint8_t TPE_jointEnvironmentResolveCollision(TPE_Joint* joint, TPE_Unit elasticity, TPE_Unit friction,
                                             TPE_ClosestPointFunction env);

/** Tests whether a body is currently colliding with the environment. */
uint8_t TPE_bodyEnvironmentCollide(const TPE_Body* body, TPE_ClosestPointFunction env);
...
uint8_t TPE_bodyEnvironmentResolveCollision(TPE_Body* body, TPE_ClosestPointFunction env);
...
uint8_t TPE_checkOverlapAABB(TPE_Vec3 v1Min, TPE_Vec3 v1Max, TPE_Vec3 v2Min, TPE_Vec3 v2Max);
...
uint8_t TPE_bodiesResolveCollision(TPE_Body* b1, TPE_Body* b2, TPE_ClosestPointFunction env);
...
/** Casts a ray against environment and returns the closest hit of a surface. If
  ...
  surface. */
TPE_Vec3 TPE_castEnvironmentRay(TPE_Vec3 rayPos, TPE_Vec3 rayDir, TPE_ClosestPointFunction environment,
                                TPE_Unit insideStepSize, TPE_Unit rayMarchMaxStep, uint32_t maxSteps);
...
/** Casts a ray against bodies in a world (ignoring the environment), returns
  ...
  ignored). */
TPE_Vec3 TPE_castBodyRay(TPE_Vec3 rayPos, TPE_Vec3 rayDir, int16_t excludeBody, const TPE_World* world,
                         int16_t* bodyIndex, int16_t* jointIndex);
...
/** Draws a debug view of a 3D physics world using a provided pixel drawing
  ...
  e.g. 16). */
void TPE_worldDebugDraw(TPE_World* world, TPE_DebugDrawFunction drawFunc, TPE_Vec3 camPos, TPE_Vec3 camRot,
                        TPE_Vec3 camView, uint16_t envGridRes, TPE_Unit envGridSize);
...
void TPE_makeBox(TPE_Joint joints[8], TPE_Connection connections[16], TPE_Unit width, TPE_Unit depth, TPE_Unit height,
                 TPE_Unit jointSize);
void TPE_makeCenterBox(TPE_Joint joints[9], TPE_Connection connections[18], TPE_Unit width, TPE_Unit depth,
                       TPE_Unit height, TPE_Unit jointSize);
void TPE_makeRect(TPE_Joint joints[4], TPE_Connection connections[6], TPE_Unit width, TPE_Unit depth,
                  TPE_Unit jointSize);
void TPE_makeTriangle(TPE_Joint joints[3], TPE_Connection connections[3], TPE_Unit sideLength, TPE_Unit jointSize);
void TPE_makeCenterRect(TPE_Joint joints[5], TPE_Connection connections[8], TPE_Unit width, TPE_Unit depth,
                        TPE_Unit jointSize);
void TPE_makeCenterRectFull(TPE_Joint joints[5], TPE_Connection connections[10], TPE_Unit width, TPE_Unit depth,
                            TPE_Unit jointSize);
void TPE_make2Line(TPE_Joint joints[2], TPE_Connection connections[1], TPE_Unit length, TPE_Unit jointSize);
...
/** Environment function for triagnular prism, e.g. for ramps. The sides array
  ...
  2).*/
TPE_Vec3 TPE_envAATriPrism(TPE_Vec3 point, TPE_Vec3 center, const TPE_Unit sides[6], TPE_Unit depth, uint8_t direction);
...
#define TPE_ENV_START(test, point)                         \
    TPE_Vec3 _pBest = test, _pTest;                        \
    TPE_Unit _dBest = TPE_DISTANCE(_pBest, point), _dTest; \
    (void)(_pBest);                                        \
    (void)(_dBest);                                        \
    (void)(_dTest);                                        \
    (void)(_pTest); // supress war
...
#define TPE_ENV_NEXT(test, point)                                              \
    {                                                                          \
        if (_pBest.x == point.x && _pBest.y == point.y && _pBest.z == point.z) \
            return _pBest;                                                     \
        _pTest = test;                                                         \
        _dTest = TPE_DISTANCE(_pTest, point);                                  \
        if (_dTest < _dBest)                                                   \
        {                                                                      \
            _pBest = _pTest;                                                   \
            _dBest = _dTest;                                                   \
...
#define ANTI_OVERFLOW 25000
    if (v.x < ANTI_OVERFLOW && v.x > -1 * ANTI_OVERFLOW && v.y < ANTI_OVERFLOW && v.y > -1 * ANTI_OVERFLOW
                  ...
                  uint8_t connectionCount, TPE_Unit mass)
{
    ...
    {
        TPE_Unit d = TPE_DISTANCE(joints[connections[i].joint1].position, joints[connections[i].joint2].position);
...
void TPE_worldInit(TPE_World* world, TPE_Body* bodies, uint16_t bodyCount, TPE_ClosestPointFunction environmentFunction)
{
    world->bodies              = bodies;
                  ...
                  TPE_Unit jointSize)
{
    ...
    for (uint8_t i = 0; i < 4; ++i)
        joints[i] = TPE_joint(TPE_vec3((i % 2) ? -1 * width : width, 0, (i / 2) ? -1 * depth : depth), jointSize);

    C(0, 0, 1) C(1, 0, 2) C(2, 3, 1) C(3, 3, 2) C(4, 0, 3) C(5, 1, 2)
}
...
void TPE_makeTriangle(TPE_Joint joints[3], TPE_Connection connections[3], TPE_Unit sideLength, TPE_Unit jointSize)
{
    joints[0] = TPE_joint(TPE_vec3(sideLength / 2, 0, TPE_sqrt((sideLength * sideLength) / 2) / 2), jointSize);
                 ...
                 TPE_Unit jointSize)
{
    ...
    for (uint8_t i = 0; i < 8; ++i)
        joints[i] = TPE_joint(TPE_vec3((i % 2) ? width : (-1 * width), ((i >> 2) % 2) ? height : (-1 * height),
                                       ((i >> 1) % 2) ? depth : (-1 * depth)),
                              jointSize);
...
void TPE_worldStep(TPE_World* world)
{
    ...
    for (uint16_t i = 0; i < world->bodyCount; ++i)
    {
        ...
        uint8_t collided = TPE_bodyEnvironmentResolveCollision(body, world->environmentFunction);
        ...
        if (body->flags & TPE_BODY_FLAG_NONROTATING)
        {
            ...
            if (collided && TPE_bodyEnvironmentCollide(body, world->environmentFunction))
        ...
        else // normal, rotating bodies
        {
            ...
            for (uint16_t j = 0; j < body->connectionCount; ++j) // joint tension
            {
                ...
                TPE_Unit tension = TPE_connectionTension(TPE_LENGTH(dir), connection->length);

                bodyTension += tension > 0 ? tension : -tension;

                if (tension > TPE_TENSION_ACCELERATION_THRESHOLD || tension < -1 * TPE_TENSION_ACCELERATION_THRESHOLD)
                {
...
TPE_Unit TPE_bodyGetNetSpeed(const TPE_Body* body)
{
    ...
    for (uint16_t i = 0; i < body->jointCount; ++i)
    {
        velocity += TPE_LENGTH(TPE_vec3(joint->velocity[0], joint->velocity[1], joint->velocity[2]));
...
void TPE_bodyMultiplyNetSpeed(TPE_Body* body, TPE_Unit factor)
{
    ...
    {
        for (uint8_t k = 0; k < 3; ++k)
            joint->velocity[k] = (((TPE_Unit)joint->velocity[k]) * factor) / TPE_F;
...
void TPE_bodyLimitAverageSpeed(TPE_Body* body, TPE_Unit speedMin, TPE_Unit speedMax)
{
    for (uint8_t i = 0; i < 16; ++i)
    {
        ...
        TPE_Unit fraction = (((speedMax + speedMin) / 2) * TPE_F) / TPE_nonZero(speed);
...
void TPE_bodyCancelOutVelocities(TPE_Body* body, uint8_t strong)
{
    for (uint16_t i = 0; i < body->connectionCount; ++i)
    {
        ...
        if (strong)
        {
            ...
            cancel
                = tension <= TPE_TENSION_ACCELERATION_THRESHOLD && tension >= -1 * TPE_TENSION_ACCELERATION_THRESHOLD;
        }
...
TPE_Vec3 TPE_bodyGetRotation(const TPE_Body* body, uint16_t joint1, uint16_t joint2, uint16_t joint3)
{
    return TPE_rotationFromVecs(TPE_vec3Minus(body->joints[joint2].position, body->joints[joint1].position),
                                TPE_vec3Minus(body->joints[joint3].position, body->joints[joint1].position));
}
...
TPE_Vec3 _TPE_rotateByAxis(TPE_Vec3 p, TPE_Vec3 axisNormalized, TPE_Unit angle)
{
    ...
    return TPE_vec3Plus(projected, TPE_vec3Plus(TPE_vec3Times(a, TPE_cos(angle)), TPE_vec3Times(b, TPE_sin(angle))));
}
...
void TPE_bodyRotateByAxis(TPE_Body* body, TPE_Vec3 rotation)
{
    ...
    for (uint16_t i = 0; i < body->jointCount; ++i)
    {
        ...
        body->joints[i].position = TPE_vec3Plus(bodyCenter, _TPE_rotateByAxis(toPoint, rotation, angle));
    }
}
...
{
    for (uint16_t i = 0; i < body->jointCount; ++i)
        body->joints[i].position = TPE_vec3Plus(body->joints[i].position, offset);
}

void TPE_bodyApplyGravity(TPE_Body* body, TPE_Unit downwardsAccel)
{
    if ((body->flags & TPE_BODY_FLAG_DEACTIVATED) || (body->flags & TPE_BODY_FLAG_DISABLED))
        return;
...
void _TPE_bodyNonrotatingJointCollided(TPE_Body* b, int16_t jointIndex, TPE_Vec3 origPos, uint8_t success)
{
...
#define _PI2 5053 // 9.8696044 * TPE_F
    return sign * // Bhaskara's approximation
           (((32 * x * _PI2) / TPE_F) * tmp) / ((_PI2 * (5 * TPE_F - (8 * x * tmp) / TPE_F)) / TPE_F);
#undef _PI2
}
...
uint8_t TPE_bodiesResolveCollision(TPE_Body* b1, TPE_Body* b2, TPE_ClosestPointFunction env)
{
    ...
    for (uint16_t i = 0; i < b1->jointCount; ++i)
        for (uint16_t j = 0; j < b2->jointCount; ++j)
        {
            ...
            if (TPE_jointsResolveCollision(&(b1->joints[i]), &(b2->joints[j]), b1->jointMass, b2->jointMass,
                                           (b1->elasticity + b2->elasticity) / 2, (b1->friction + b2->friction) / 2,
                                           env))
            {
                r = 1;
                                   ...
                                   TPE_Unit friction, TPE_ClosestPointFunction env)
{
    ...
    if (d < 0) // collision?
    {
        if (_TPE_collisionCallback != 0
            && !_TPE_collisionCallback(_TPE_body1Index, _TPE_joint1Index, _TPE_body2Index, _TPE_joint2Index,
                                       TPE_vec3Plus(j1->position, dir)))
            return 0;

        TPE_Vec3 pos1Backup = j1->position, pos2Backup = j2->position;
        ...
        TPE_Unit ratio = (mass2 * TPE_F) / TPE_nonZero(mass1 + mass2);
        ...
        frictionVec = TPE_vec3Minus(TPE_vec3(j2->velocity[0], j2->velocity[1], j2->velocity[2]), frictionVec);
...
#define assignVec(j, i, d, o) \
    j->velocity[i] = j->velocity[i] + vel.d o(((frictionVec.d * ratio) / TPE_F) * friction) / TPE_F;

        assignVec(j1, 0, x, +) assignVec(j1, 1, y, +) assignVec(j1, 2, z, +)

            vel
            = TPE_vec3Times(dir, v2);

        ratio = TPE_F - ratio;

        assignVec(j2, 0, x, -) assignVec(j2, 1, y, -) assignVec(j2, 2, z, -)
...
void TPE_getVelocitiesAfterCollision(TPE_Unit* v1, TPE_Unit* v2, TPE_Unit m1, TPE_Unit m2, TPE_Unit elasticity)
{
    ...
    *v1 = (((elasticity * m2 / TPE_F) * v2Mv1) + m1v1Pm2v2) / m1Pm2;

    *v2 = (((elasticity * m1 / TPE_F) * -1 * v2Mv1) + m1v1Pm2v2) / m1Pm2;
}
                                             ...
                                             TPE_ClosestPointFunction env)
{
    ...
    {
        if (_TPE_collisionCallback != 0)
            if (!_TPE_collisionCallback(_TPE_body1Index, _TPE_joint1Index, _TPE_body2Index, _TPE_joint2Index,
        ...
        if (len > 0)
        {
            ...
            for (int i = 0; i < TPE_COLLISION_RESOLUTION_ITERATIONS; ++i)
            {
                ...
                shift = TPE_vec3Times(shift, TPE_JOINT_SIZE(*joint) - len + TPE_COLLISION_RESOLUTION_MARGIN);
        ...
        if (!success)
        {
            ...
            shift = TPE_vec3(-1 * joint->velocity[0], -1 * joint->velocity[1], -1 * joint->velocity[2]);
        ...
        if (success)
        {
            TPE_Vec3 vel = TPE_vec3(joint->velocity[0], joint->velocity[1], joint->velocity[2]);
...
void TPE_bodyGetBSphere(const TPE_Body* body, TPE_Vec3* center, TPE_Unit* radius)
{
    ...
    *center = TPE_bodyGetCenterOfMass(body);
    ...
    for (uint16_t i = 0; i < body->jointCount; ++i)
    {
        ...
        diff.x = ((center->x > j->position.x) ? (center->x - j->position.x) : (j->position.x - center->x)) + js;

        diff.y = ((center->y > j->position.y) ? (center->y - j->position.y) : (j->position.y - center->y)) + js;

        diff.z = ((center->z > j->position.z) ? (center->z - j->position.z) : (j->position.z - center->z)) + js;
...
uint8_t TPE_bodyEnvironmentResolveCollision(TPE_Body* body, TPE_ClosestPointFunction env)
{
    ...
    for (uint16_t i = 0; i < body->jointCount; ++i)
    {
        ...
        uint8_t r = TPE_jointEnvironmentResolveCollision(body->joints + i, body->elasticity, body->friction, env);
...
TPE_Unit TPE_atan(TPE_Unit x)
{
    ...
    return sign * (307 * x + x2) / ((267026 + 633 * x + x2) / 128);
}
...
TPE_Vec3 TPE_rotationFromVecs(TPE_Vec3 forward, TPE_Vec3 right)
{
    ...
    result.x = TPE_vec2Angle(forward.z, forward.y);
...
void _TPE_drawDebugPixel(TPE_Unit x, TPE_Unit y, TPE_Unit w, TPE_Unit h, uint8_t c, TPE_DebugDrawFunction f)
{
    if (x >= 0 && x < w && y >= 0 && y < h)
        f(x, y, c);
}
            ...
            for (uint8_t k = 0; k < envGridRes; ++k)
            {
                ...
                for (uint8_t l = 0; l < envGridRes; ++l)
                {
                    ...
                    if (r.x != testPoint.x || r.y != testPoint.y || r.z != testPoint.z)
                    {
                        ...
                        if (r.z > Z_LIMIT)
                            _TPE_drawDebugPixel(r.x, r.y, camView.x, camView.y, TPE_DEBUG_COLOR_ENVIRONMENT, drawFunc);
                    }
...
#define SEGS 16
            ...
            uint8_t c = (world->bodies[i].flags & TPE_BODY_FLAG_DEACTIVATED) ? TPE_DEBUG_COLOR_INACTIVE
                                                                             : TPE_DEBUG_COLOR_CONNECTION;
...
#undef SEGS
        ...
        {
            TPE_Vec3 p = _TPE_project3DPoint(world->bodies[i].joints[j].position, camPos, camRot, camView);

            if (p.z > Z_LIMIT)
            {
                uint8_t color = (world->bodies[i].flags & TPE_BODY_FLAG_DEACTIVATED) ? TPE_DEBUG_COLOR_INACTIVE
                                                                                     : TPE_DEBUG_COLOR_JOINT;
...
#define SEGS 4
                ...
                {
                    TPE_Unit dx = (TPE_sin(TPE_F * k / (8 * SEGS)) * size) / TPE_F,
                             dy = (TPE_cos(TPE_F * k / (8 * SEGS)) * size) / TPE_F;

#define dp(a, b, c, d) _TPE_drawDebugPixel(p.x a b, p.y c d, camView.x, camView.y, color, drawFunc);
                    dp(+, dx, +, dy) dp(+, dx, -, dy) dp(-, dx, +, dy) dp(-, dx, -, dy) dp(+, dy, +, dx)
                        dp(+, dy, -, dx) dp(-, dy, +, dx) dp(-, dy, -, dx)
#undef dp
#undef SEGS
                }
            }
...
TPE_Vec3 TPE_envBox(TPE_Vec3 point, TPE_Vec3 center, TPE_Vec3 maxCornerVec, TPE_Vec3 rotation)
{
    point = TPE_pointRotate(TPE_vec3Minus(point, center), TPE_rotationInverse(rotation));

    return TPE_vec3Plus(center, TPE_pointRotate(TPE_envAABox(point, TPE_vec3(0, 0, 0), maxCornerVec), rotation));
}
...
TPE_Vec3 TPE_envAABox(TPE_Vec3 point, TPE_Vec3 center, TPE_Vec3 maxCornerVec)
{
    ...
    uint8_t region
        = (shifted.x > maxCornerVec.x) | ((shifted.y > maxCornerVec.y) << 1) | ((shifted.z > maxCornerVec.z) << 2);
...
TPE_Vec3 TPE_envAABoxInside(TPE_Vec3 point, TPE_Vec3 center, TPE_Vec3 size)
{
    ...
    TPE_Vec3 a = TPE_vec3Minus(size, shifted), b = TPE_vec3Plus(shifted, size);
...
#define test(c)                                       \
    ...
    if (dist < 0)                                     \
        dist *= -1;                                   \
    if (dist > v1Max.c - v1Min.c + v2Max.c - v2Min.c) \
        return 0;
...
TPE_Vec3 TPE_vec3KeepWithinDistanceBand(TPE_Vec3 point, TPE_Vec3 center, TPE_Unit minDistance, TPE_Unit maxDistance)
{
    ...
    return TPE_vec3Plus(center, TPE_vec3Times(TPE_vec3Normalized(toPoint), l));
}

TPE_Vec3 TPE_vec3KeepWithinBox(TPE_Vec3 point, TPE_Vec3 boxCenter, TPE_Vec3 boxMaxVect)
{
    point.x = TPE_keepInRange(point.x, boxCenter.x - boxMaxVect.x, boxCenter.x + boxMaxVect.x);

    point.y = TPE_keepInRange(point.y, boxCenter.y - boxMaxVect.y, boxCenter.y + boxMaxVect.y);

    point.z = TPE_keepInRange(point.z, boxCenter.z - boxMaxVect.z, boxCenter.z + boxMaxVect.z);
...
TPE_Vec3 TPE_envCylinder(TPE_Vec3 point, TPE_Vec3 center, TPE_Vec3 direction, TPE_Unit radius)
{
    ...
    if (lDiff > 0)
    {
        ...
        point = (TPE_vec3Dot(projected, direction)) >= 0 ? TPE_vec3Minus(point, direction)
                                                         : TPE_vec3Plus(point, direction);
    }
...
TPE_Vec3 TPE_fakeSphereRotation(TPE_Vec3 position1, TPE_Vec3 position2, TPE_Unit radius)
{
    ...
    TPE_Unit d = (TPE_DISTANCE(position1, position2) * TPE_F) / (radius * 4);
                                ...
                                TPE_Unit insideStepSize, TPE_Unit rayMarchMaxStep, uint32_t maxSteps)
{
    ...
    if (p2.x != p.x || p2.y != p.y || p2.z != p.z)
    {
        ...
        for (uint32_t i = 0; i < maxSteps; ++i)
        {
            ...
            if (d == 0 || (p2.x == p.x && p2.y == p.y && p2.z == p.z))
    ...
    if (found)
    {
        ...
        for (uint8_t i = 0; i < 128; ++i) // upper limit just in case
        {
            ...
            if ((found == 1) == (pTest.x == middle.x && pTest.y == middle.y && pTest.z == middle.z))
                p2 = middle;
            else
                p = middle;
        }
                ...
                for (uint16_t j = 0; j < world->bodies[i].jointCount; ++j)
                {
                    ...
                    if (TPE_vec3Dot(p, rayDir) >= 0)
                    {
                        ...
                        if (d <= js)
                        {
                            ...
                            TPE_Vec3 i1 = TPE_vec3Plus(p, c), // intersection points
        ...
        if (p.x != point.x || p.y != point.y)
        {
            ...
            if ( // dot product to determine which side the point is on
                (sides[i2] - sides[i]) * (point.x - sides[i])
                    + (sides[i2 + 1] - sides[i + 1]) * (point.y - sides[i + 1])
                < 0)
            {
                point.x = sides[i];
                point.y = sides[i + 1];
                ...
                (sides[i] - sides[i2]) * (point.x - sides[i2])
                    + (sides[i + 1] - sides[i2 + 1]) * (point.y - sides[i2 + 1])
                < 0)
            {
                point.x = sides[i2];
                point.y = sides[i2 + 1];
...
uint32_t TPE_jointHash(const TPE_Joint* joint)
{
    uint32_t r = _TPE_hash(joint->position.x);
    r          = _TPE_hash(r ^ joint->position.y);
    r          = _TPE_hash(r ^ joint->position.z);
    r          = _TPE_hash(r ^ (((uint32_t)joint->velocity[0]) | (((uint32_t)joint->velocity[1]) << 16)));
    r          = _TPE_hash(r ^ (((uint32_t)joint->velocity[2]) | ((uint32_t)joint->sizeDivided)));
...
uint32_t TPE_connectionHash(const TPE_Connection* connection)
{
    return _TPE_hash(((uint32_t)connection->length) | (((uint32_t)connection->joint1) << 16)
                     | (((uint32_t)connection->joint2) << 24));
}

uint32_t TPE_bodyHash(const TPE_Body* body)
{
    uint32_t r = _TPE_hash(((uint32_t)body->jointMass) | (((uint32_t)body->flags) << 16)
                           | (((uint32_t)body->deactivateCount) << 24))
                 ^ _TPE_hash(((uint32_t)body->friction) | (((uint32_t)body->elasticity) << 16));
        ...
        for (uint16_t y = 0; y < gridResolution; ++y)
        {
            ...
            for (uint16_t x = 0; x < gridResolution; ++x)
            {
                ...
                if (p.x != p2.x || p.y != p2.y || p.z != p2.z) // only test outside
                {
                    ...
                    {
                        p3 = TPE_vec3((p3.x + p2.x) / 2, (p3.y + p2.y) / 2, (p3.z + p2.z) / 2);

                        TPE_Vec3 p4 = f(p3, TPE_INFINITY);

                        if (TPE_abs(p4.x - p2.x) + TPE_abs(p4.y - p2.y) + TPE_abs(p4.z - p2.z)
                            > allowedError) // taxicab dist. for speed
                        {
                            if (errorPoint != 0)
                                *errorPoint = p;
...
TPE_Vec3 TPE_envLineSegment(TPE_Vec3 point, TPE_Vec3 a, TPE_Vec3 b)
{
    ...
    else if (TPE_abs(point.x) + TPE_abs(point.y) + TPE_abs(point.z) > TPE_abs(b.x) + TPE_abs(b.y) + TPE_abs(b.z))
        point = b;
                          ...
                          TPE_Unit (*heightFunction)(int32_t x, int32_t y), TPE_Unit maxDist)
{
    ...
    int16_t startSquareX = point.x / gridSize - (point.x < 0), startSquareY = point.z / gridSize - (point.z < 0);

    int16_t squareX = startSquareX, squareY = startSquareY;
    ...
    TPE_Vec3 // 4 corners of the current square
        bl
        = TPE_vec3(squareX * gridSize, heightFunction(squareX, squareY), squareY * gridSize),
    ...
    {
        if ((TPE_min(TPE_abs(squareX - startSquareX), TPE_abs(squareY - startSquareY)) - 1) * gridSize
        ...
        {
            TPE_Vec3 testP = TPE_envHalfPlane(
                point, j == 0 ? bl : tr,
                TPE_vec3Normalized(j == 0 ? TPE_vec3Cross(TPE_vec3Minus(tl, bl), TPE_vec3Minus(br, bl))
...
#define testEdge(a, b)                       \
    testP = TPE_envLineSegment(point, a, b); \
    testD = TPE_DISTANCE(testP, point);      \
    if (testD < closestD)                    \
    {                                        \
        closestP = testP;                    \
        closestD = testD;                    \
                    ...
                    testEdge(j == 0 ? bl : tr, br) testEdge(j == 0 ? bl : tr, tl) testEdge(br, tl)
