...
#ifdef S3L_EXTERN_RESOLUTION_X
    uint16_t S3L_resolutionX = 512;
#endif

#ifdef S3L_EXTERN_RESOLUTION_Y
    uint16_t S3L_resolutionY = 512;
#endif

static window3d_t windowConfig;

#ifndef S3L_PIXEL_FUNCTION
  #define S3L_PIXEL_FUNCTION swadgePixelFunction
#endif

/**
 * @brief Configures small3dlib to draw directly onto the screen
 *
 * @param x The X offset to begin drawing on the screen
 * @param y The Y offset to begin drawing on the screen
 * @param w The width of the area to draw within
 * @param h The height of the area to draw within
 */
void configureS3dScreen(int x, int y, int w, int h)
{
  ...
  windowConfig.w = w;
  windowConfig.h = h;
  windowConfig.fb = NULL;
}

void configureS3dCallback(int w, int h, s3dPixelCallback_t cb)
{
  S3L_resolutionX = w;
  S3L_resolutionY = h;
...
/**
 * @brief Configures small3dlib to draw onto the supplied framebuffer instead of directly to the screen
 *
 * @param w The width of the framebuffer
 * @param h The height of the framebuffer
 * @param fb A pointer to an array of paletteColor_t with a size of at least w*h
 */
...
static inline void swadgePixelFunction(S3L_PixelInfo *pixel)
{
  ...
  if (windowConfig.cb)
  {
    windowConfig.cb(pixel);
  }
  else if (windowConfig.fb)
  {
    windowConfig.fb[S3L_resolutionY * pixel->y + pixel->x] = c555;
  }
  else
  {
    TURBO_SET_PIXEL(windowConfig.x + pixel->x, windowConfig.y + pixel->y, c555);
  }
}
