 ...
 * set pitches that can work nicely with background music. \section cutscene Usage
 *
 * Start with initCutscene(), then add any number of styles with addCutsceneStyle(). Generally think of a style as a
 * character. It has its name, various poses, and a certain sound. Styles can also be used in other clever ways such as
 * an incoming transmission, or getting a new ability!
 *
 * Consider calling setMidiParams() in a way that meshes with other background music. You can call it again when the BGM
 * changes, and all the parameters will be overwritten. But this function is completely optional. Midi sounds will be
 * populated with some kind of noise by default.
 *
 * Add any number of dialogue lines with addCutsceneLine(). A list will be internally shifted as each line is disposed
 * with the A press. At any point later in your game, simply add a bunch of other dialogue lines to do yet another
 * cutscene.
 *
...
/**
 * @brief The underlying data for a cutscene. A cutscene is fundamentally made of a list of cutsceneLine_t. With other
 * variables that can be stored in one place here.
 */
typedef struct
{
    ...
    uint8_t soundBank;              ///< 2 for mega man sounds.
                                    ///< https://adam.feinste.in/Super-2024-Swadge-FW/MIDI.html#general-midi-bank-0
    ...
    int16_t songPitches[8]; ///< Up to eight pitches and no less than four pitches that harmonize with a song. Any
                            ///< pitches in idx 4 thru 7 may be -1 to be unused.
} cutscene_t;
...
void addCutsceneStyle(cutscene_t* cutscene, paletteColor_t color, cnfsFileIdx_t spriteIdx, cnfsFileIdx_t textBoxIdx,
                      char* title, uint8_t numPoseVariations, bool stageLeft, bool drawSprite, bool drawTextbox);
