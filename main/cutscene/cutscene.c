...
/**
 * @brief Removes all styles. Unlikely to be used by anybody else since you add all your styles at the start of the
 * game.
 *
 * @param cutscene Pointer to the cutscene_t
 */
 ...
 * @param drawTextbox If true, then the textbox sprite is drawn behind the text. Use false to draw just the text with no
 * textbox.
 */
void addCutsceneStyle(cutscene_t* cutscene, paletteColor_t textColor, cnfsFileIdx_t spriteIdx, cnfsFileIdx_t textBoxIdx,
                      char* title, uint8_t numSpriteVariations, bool stageLeft, bool drawSprite, bool drawTextbox)
{
 ...
 *
 * @param cutscene Pointer to the cutscene_t
 * @param songPitches No less than four midi pitches and no more than 8 that may play well against other background
 * music. Any -1's will be ignored in the array.
 */
                     ...
                     cutsceneCb cbFunc)
{
    ...
    if (cutscene->lines->first == NULL)
    {
        ...
        loadWsg(style->spriteIdx + line->spriteVariation, cutscene->sprite, true);
...
void updateCutscene(cutscene_t* cutscene, int16_t btnState)
{
    ...
    // Extra hold required because sometimes click-unclick-click registers on the hardware from an intended single
    // click.
...
/**
 * @brief The draw function of the cutscene must be called regularly from your draw loop. Update should typically be
 * called before draw.
 *
 * @param cutscene Pointer to the cutscene_t
...
/**
 * @brief Frees data. Required to call where your mode exits or when done with cutscene_t. Remember you may recycle one
 * cutscene_t for many cutscenes and deinit during Mode Exit.
 *
 * @param cutscene
 */
