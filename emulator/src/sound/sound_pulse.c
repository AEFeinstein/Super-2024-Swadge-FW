...
void* InitSoundPulse(SoundCBType cb, int reqSPS, int reqChannelsRec, int reqChannelsPlay, int sugBufferSize,
                     const char* inputSelect, const char* outputSelect)
{
    ...
    printf("Pulse: from: %s/%s (%s) / %dx(%d,%d) (%d)\n", r->sourceNameRec, r->sourceNamePlay, title, r->sps,
    ...
    if (r->channelsPlay)
    {
        ...
        int ret           = pa_stream_connect_playback(r->play, r->sourceNameRec, &bufattr,
                                                       ...
                                                       // |PA_STREAM_ADJUST_LATENCY //Some servers don't like the adjust_latency
                                                       // flag. |PA_STREAM_AUTO_TIMING_UPDATE, NULL, NULL);
                                                       0, NULL, NULL);
        if (ret < 0)
        {
    ...
    if (r->channelsRec)
    {
        ...
        int ret           = pa_stream_connect_record(
            r->rec, r->sourceNamePlay, &bufattr,
            0
                //							       |PA_STREAM_INTERPOLATE_TIMING
