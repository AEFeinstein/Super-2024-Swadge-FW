...
static int SetHWParams(snd_pcm_t* handle, int* samplerate, short* channels, snd_pcm_uframes_t* bufsize,
                       struct SoundDriverAlsa* a)
{
    int err;
...
static struct SoundDriverAlsa* InitASound(struct SoundDriverAlsa* r)
{
    ...
    if (r->channelsPlay)
    {
        if ((err = snd_pcm_open(&r->playback_handle, r->devPlay ? r->devPlay : "default", SND_PCM_STREAM_PLAYBACK, 0))
            < 0)
        {
