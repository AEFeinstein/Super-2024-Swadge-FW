                              ...
                              int reqChannelsPlay, int sugBufferSize, const char* inputSelect, const char* outputSelect)
{
    ...
    if (driver_name == 0 || strlen(driver_name) == 0)
    {
        ...
        for (i = 0; i < MAX_SOUND_DRIVERS; i++)
        {
            ...
            ret = SoundDrivers[i](cb, reqSPS, reqChannelsRec, reqChannelsPlay, sugBufferSize, inputSelect,
                                  outputSelect);
            if (ret)
            {
                return ret;
    ...
    else
    {
        ...
        for (i = 0; i < MAX_SOUND_DRIVERS; i++)
        {
            ...
            {
                return SoundDrivers[i](cb, reqSPS, reqChannelsRec, reqChannelsPlay, sugBufferSize, inputSelect,
                                       outputSelect);
            }
        }
    }
