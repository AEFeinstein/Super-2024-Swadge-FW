...
static struct SoundDriverAndroid* InitAndroidSound(struct SoundDriverAndroid* r)
{
    ...
    if (r->channelsPlay)
    {
        ...
        SLDataFormat_PCM format_pcm = {
            SL_DATAFORMAT_PCM,           r->channelsPlay,
            SL_SAMPLINGRATE_16,          SL_PCMSAMPLEFORMAT_FIXED_16,
            SL_PCMSAMPLEFORMAT_FIXED_16, (r->channelsPlay == 1) ? SL_SPEAKER_FRONT_CENTER : 3,
        ...
        result
            = (*r->playerObject)->GetInterface(r->playerObject, SL_IID_ANDROIDSIMPLEBUFFERQUEUE, &r->playerBufferQueue);
        assert(SL_RESULT_SUCCESS == result);
        (void)result;
    ...
    if (r->channelsRec)
    {
        ...
        SLDataLocator_IODevice loc_devI
        ...
        result = (*r->engineEngine)
                     ->CreateAudioRecorder(r->engineEngine, &r->recorderObject, &audioSrc, &audioSnk, 1, id, req);
        if (SL_RESULT_SUCCESS != result)
        {
        ...
        result = (*r->recorderObject)
                     ->GetInterface(r->recorderObject, SL_IID_ANDROIDSIMPLEBUFFERQUEUE, &r->recorderBufferQueue);
        assert(SL_RESULT_SUCCESS == result);
        (void)result;
